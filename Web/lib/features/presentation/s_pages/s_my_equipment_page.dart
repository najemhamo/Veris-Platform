...
class SGetMyEquipmentsPage extends StatefulWidget {
  final String uid;
  const SGetMyEquipmentsPage({Key? key,required this.uid}) : super(key: key);

  @override
  _SGetMyEquipmentsPageState createState() => _SGetMyEquipmentsPageState();
}

class _SGetMyEquipmentsPageState extends State<SGetMyEquipmentsPage> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<EquipmentCubit,EquipmentState>(builder: (context, equipmentState) {
      if (equipmentState is EquipmentLoaded) {

        final myEquipments=equipmentState.equipments.where((element) => element.queue!.contains(widget.uid)).toList();
        final filteredEquipment = myEquipments.where((ep) =>
        ep.name!.startsWith(_searchController.text) ||
            ep.name!.startsWith(_searchController.text) ||
            ep.name!.toLowerCase().startsWith(_searchController.text.toLowerCase())
            || ep.description!.startsWith(_searchController.text) ||
            ep.description!.startsWith(_searchController.text) ||
            ep.description!.toLowerCase().startsWith(_searchController.text.toLowerCase())
        ...
        return Column(
          children: [
            ...
            Expanded(
              child: filteredEquipment.isEmpty?Center(child: Text("No Equipment added yet")):GridView.builder(
                ...
                itemBuilder: (context, index) {
                  return InkWell(
                    onTap: (){
                     _equipmentNameController.value=TextEditingValue(text: filteredEquipment[index].name!);
                       _shelfController.value = TextEditingValue(text: filteredEquipment[index].shelf!);
                       _quantityController.value = TextEditingValue(text: filteredEquipment[index].quantity.toString());
                       _descriptionController.value = TextEditingValue(text: filteredEquipment[index].description!);
                    ...
                    child: Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          SizedBox(height: 4,),
                          Text("${filteredEquipment[index].name}",style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold),),
                          SizedBox(height: 4,),
                          Text("${filteredEquipment[index].description}",maxLines: 5,style: TextStyle(fontSize: 14,fontWeight: FontWeight.w500),overflow: TextOverflow.ellipsis,),
                        ],
                      ),
                    ),
                  );
          ...
          builder: (BuildContext context, void Function(void Function()) setState) {
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  SizedBox(height: 10,),
                  _profileWidget(profile: equipment.equipmentPhoto,setState: setState),
                  SizedBox(height: 10,),
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          Text("${(equipment.quantity!.toInt() - equipment.queue!.length.toInt())}")
                        ],
                      ),
                    ],
                  ),
